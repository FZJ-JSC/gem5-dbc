...
from g5dbc.sim.model.board.BoardSystemSE import BoardSystemSE
    ...
    def create(config: Config) -> AbstractBoardSystem:
        ...
        if config.simulation.full_system:
            match config.system.platform:
                ...
                case _:
                    raise ValueError(
                        f"Platform model {config.system.platform} not available"
                    )
            return ArmBoardSystem(
                config=config,

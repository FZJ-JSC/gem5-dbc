...
from g5dbc.sim.m5_objects import m5_AddrRange, m5_Addr
from g5dbc.sim.m5_objects.arch import m5_ArmSystem, m5_ArmFsLinux
...
class ArmBoardSystem(m5_ArmSystem, AbstractBoardSystem):
    ...
    def create_memory_ranges(self, sorted_regions: list[MemoryRegionConfig]) -> list[m5_AddrRange]:
        #@TODO: Assumes VExpress_GEM5_Base platform
    ...
    def configure_system_boot(self, config: Config):
        ...
        artifact_path = Path(config.artifacts.path)
        kernels       = config.artifacts.search_type("KERNEL")
        bootloaders   = config.artifacts.search_type("BOOT")
        disk_images   = config.artifacts.search_type("DISK")

        bootloader_path = [v.path for k,v in bootloaders.items() if v.metadata == config.system.platform ]
        ...
        kernel_path     = str(artifact_path.joinpath(kernels["vmlinux"].path))
        # Use first bootloader in list
        bootloader_path = str(artifact_path.joinpath(bootloader_path[0]))
    ...
    def attach_disk_images(self, config: Config):
        ...
        artifact_path = Path(config.artifacts.path)
        disk_images   = config.artifacts.search_type("DISK")
        paths = [ str(artifact_path.joinpath(v.path))  for k,v in disk_images.items()]
    ...
    def attach_pci_devices(self, config: Config):
        ...
        for dev in self._pci_devices:
            self.realview.attachPciDevice(
                dev,
                self.iobus,
                dma_ports=self._dma_ports if is_active else None)

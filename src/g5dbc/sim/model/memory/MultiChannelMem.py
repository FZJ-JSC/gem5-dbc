...
class MultiChannelMem:
    ...
    def __init__(self, config: MemoryRegionConfig, mem_ctrls: list[AbstractMemCtrl]) -> None:
        """
        """
        self._config = config
    ...
    def set_memory_range(self, range: m5_AddrRange) -> None:
        ...
        xor_high_bit = self._xor_low_bit + intlv_bits - 1 if self._xor_low_bit > 0 else 0

        for idx, ctrl in enumerate(self._ctrls):
            ctrl.set_addr_range(m5_AddrRange(
                start = range.start,
                size  = range.size(),

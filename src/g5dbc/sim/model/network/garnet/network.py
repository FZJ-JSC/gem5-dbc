...
from g5dbc.sim.m5_objects.ruby.network import m5_GarnetNetwork, m5_GarnetNetworkInterface
from g5dbc.sim.model.topology import AbstractTopology
from g5dbc.sim.model.topology.spec import NodeSpec, RouterSpec, LinkSpec
...
from .link import Garnet_IntLink, Garnet_ExtLink
...
class Garnet_Network(m5_GarnetNetwork, AbstractNetwork):
    ...
    def connect_interfaces(self):  
        ...
        self.netifs = [m5_GarnetNetworkInterface(id=i) for (i,link) in enumerate(self._ext_links)]
    ...
    def create_link(self, link: LinkSpec, vnets: list[int]):
        ...
        int_links.append(Garnet_IntLink(
                link_id = link_id,
    ...
    def connect_node(self, node: AbstractNode, mesh_lvs: list[list[int]]):
        ...
        for ctrl in node.get_controllers():
            for vnets in mesh_lvs:
                ...
                ext_links.append(Garnet_ExtLink(
                    link_id = link_id,
                    ext_node = ctrl,
                    int_node = router,
    ...
    def get_num_virtual_networks(self) -> int:
        return self.number_of_virtual_networks

...
def write_scatterplot_hline(filename: str, xy_values: dict, line_values: dict, plot_params: dict, dpi=600):
    ...
    for label in labels: #, values in xy_values.items():
        ...
        h = line_values[label]
        ax.plot(x, y, '-', color=next(line_color), label=f"{label} {h:.2f}")
    ...
    for label in labels:
        y = line_values[label]
        ax2.axhline(y=y, color=next(line_color), label=label)

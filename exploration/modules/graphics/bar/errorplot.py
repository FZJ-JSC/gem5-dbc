...
def plot_errorbarplot(filename: str, xy_values: dict, plot_params: dict, dpi=600):
    ...
    v_ticks   = sorted(next(iter(xy_values.values()))['x_labels'])
    nbars     = len(labels)
    ...
    for i, l in enumerate(labels):
        ...
        plot_y = xy_values[l]['mean']
        yerr = xy_values[l]['std']
        label = label_fmt.format(l).replace(r'_', r'\_').replace(r'%', r'\%')
        if is_horizontal:
            bars.append(ax.barh(plot_x, plot_y, bar_width, xerr=yerr, color=next(bar_color), label=label ))
        else:
            bars.append(ax.bar( plot_x, plot_y, bar_width, yerr=yerr, color=next(bar_color), label=label ))
